...
import DeleteProductInlineButton from './DeleteProductInlineButton'
            ...
            {filtered.map((p) => (
              <tr key={String(p.id)} className="border-t border-gray-800">
                ...
                <td className="py-2">
                  ...
                  <DeleteProductInlineButton id={p.id} onDeleted={(id)=> setRows(prev => prev.filter(x => String(x.id) !== String(id)))} />
                </td>
              </tr>
            ))}
          </tbody>
